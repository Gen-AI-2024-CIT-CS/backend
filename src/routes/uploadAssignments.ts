...
assignmentUploadRouter.post("/all", upload.single("file"), async (req, res) => {
  if (!req.file) {
    return res.status(400).json({ message: "No file uploaded." });
  }
  const token = req.cookies["jwt_token"];
  ...
  // Get the absolute path of the Python script
  const scriptPath = path.join(__dirname, "../scripts/insertAllAssignments.py"); // Adjust the path if neededt
  const command = process.platform === "win32" 
  ? `py "${scriptPath}" "${userName}" "${courseID}"` 
  : `python3 "${scriptPath}" "${userName}" "${courseID}"`;
